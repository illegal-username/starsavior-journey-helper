...
1.1.0 이하 시험판은 앱 ID와 서명 체계가 다르므로 2.0.0 이상의 공개판으로 덮어쓸 수 없습니다. 기존 플로팅 아이콘을 종료하고 이전 앱을 삭제한 뒤 새 공개판을 설치해야 합니다. 오버레이·알림·화면 공유 권한도 다시 허용해야 합니다. 2.0.0 이상의 공개판 및 build45 시험판 사용자는 같은 운영 키로 서명한 2.1.10 APK를 그대로 설치해 업데이트할 수 있습니다. 내장 DB 시험판에서 전환한 후에는 앱에서 `새 DB 받기`를 눌러 정식 DB를 적용하세요.
